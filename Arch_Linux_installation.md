...
- [Install](https://wiki.archlinux.org/title/Systemd-boot#Installation) `systemd-boot` EFI boot manager.
  ...
  Use `lsblk -dno UUID /dev/nvme0n1p2` to get UUID for root partition,
  or create [label](https://wiki.archlinux.org/title/Persistent_block_device_naming#by-label).
  When using btrfs as root partition add kernel parameter [`rootflags=subvol=/@`](https://wiki.archlinux.org/title/btrfs#Mounting_subvolume_as_root).
